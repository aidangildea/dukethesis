...
The following tabs allows users without color blindess to see **duke** from their perspective and from the perspective of those who are deuteronamolous. This allows for a better understanding of how the package improves accessibility in Duke data visualizations.
...
# remove blank spaces from course variable
registrar$Course <- gsub(" ", "", registrar$Course)
```
...
Introductory courses with the most successful enrollment rates. May guide which cources to expand or add additional sections.
```{r}
#| warning: false
#| echo: false
#| message: false
...
Introductory courses with the least successful enrollment rates. May guide which courses to decrease size or remove.
