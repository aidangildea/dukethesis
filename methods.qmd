...
The ongoing aim of the **duke** package is to implement Atkinson-Hyperlegible font across all text features of visualizations.
This requires that users have the font files locally installed to their systems.
We have constructed a single-use function that downloads the necessary Atkinson-Hyperlegible files; however, it is not stable across all systems.
As we continue to craft an optimal solution, we recommend that users download [Atkinson-Hyperlegible](https://brailleinstitute.org/freefont) and then specify its use in the base family argument of `theme_duke()`.
This is not a fully streamlined solution, yet it ensures that users on all systems can implement **duke** without encountering errors.
If font in not specified, the function uses the **ggplot2** default font, sans.
