...
-   Companies can highlight their brand via R packages 
  i.e. **`ggtech`** (Airbnb, Facebook, Google), **`ggthemes`** (The Economist, The Wall Street Journal)
...
![](imgs/chromaticity.jpg){#fig-chroma}

-   To account for this irregularity, we use the color contrast ratio: $$\frac{(L1 + 0.05)} {(L2 + 0.05)}$$

## Definition of Optimal Pallette

INSERT IMAGE - Ad-hoc approach in order to strike a balance between accessibility and recognition of the Duke brand.

# Package Development Process

## `theme_duke()`

-   Set the aesthetic elements for the visualization's background and accessory features (background color, grid lines, axis ticks, plot text, etc.)
-   Minimalist design with a White background and grid lines removed
-   Text features are in in Duke Navy Blue because of its strong contrast to the White background (CR of 14.76).
-   Relative text sizing ensures consistent scale

## Font specification in `theme_duke()`

-   Font specification is **challenging**
-   Recommend the use of Atkinson Hyperlegible font
    -   Focuses on letterform distinction to increase character recognition, ultimately improving readability. (CITE)
...
-   Applies a Duke-specific continous color scale to geometric objects (geom)
-   Continuous sequences from a dark (Duke Royal Blue) to a light color (Hatteras)
    -   Duke Royal Blue has a CCR of 7.75, Hatteras has a CCR of 1.25
-   Continuous color scales used for overall trend recognition

## `scale_duke_continuous()` in use
...
## `scale_duke_color_discrete()` and `scale_duke_fill_discrete()` {.smaller}

-   Applies a Duke-specific discrete color scale for the color and fill parameters of ggplot geometric objects
...
#| label: tbl-ratios
#| echo: false
#| tbl-cap: "Pairing with five highest CCRs." 
#| tbl-align: "center"
#| cap-location: bottom
#| warning: false
...
#| fig-align: "center"
#| fig-asp: .618
#| fig-cap: "Heatmap of color contrast ratios of all pairings."
#| cap-location: bottom
#| warning: false
...
```{r}
#| label: fig-hmap-final
#| echo: false
#| fig-align: "center"
#| fig-asp: .618
#| fig-cap: "Heatmap of color contrast ratios of selected pairings."
#| cap-location: bottom
#| warning: false
#| layout-nrow: 2
...
#| fig-align: "center"
#| fig-height: 1
#| fig-cap: "Selected discrete palette."
#| cap-location: bottom
#| warning: false
...
-   Best practice to use three to five colors from discrete scale.

## `scale_duke_color_discrete()` in use
...
## `scale_duke_fill_discrete()` in use
...
-   Reduce the complexity of visualization
    -   *What do I need to highlight in the data? What are the primary insights readers should understand?*
...
-   "Get it right in black and white" [@muth2020]
-   Use labels, arrows, shapes, annotations to differentiate between categories

```{r}
#| label: fig-best-practices
#| fig-cap: "a) Scatterplot using shape. b) Line plot using different linetypes."
#| fig-subcap: 
#|   - ""
#|   - ""
#| layout-ncol: 2
...
![Bar plot with fill texture](~/Desktop/dukethesis/presentation/imgs/fillbar_2.png){#fig-fillbar}
...
- Specification of Atkinson-Hyperlegible font by default
- Ordinal scale, diverging scale 
...
- Data on all Duke courses between 2013-2022 (10 variables, 63,433 observations)
...
#| echo: false
#| warning: false
vars <- c("acad_year", "acad_org", "class_nbr", "subject", "catalog_number", "class_descr",  "total_enrollment", "enrollment_capacity", "room_capacity", "room_descr")
desc <- c("Academic year of the course.", "Academic organization abbreviation of the course; proxy for department.", "Unique class number for the course.", "Subject abbreviation for course.", "Course number within academic organizations.", "Course name.",  "Number of students enrolled in course.", "Enrollment capacity for course.", "Capacity for students permitted in course room.", "Room where course is held.")

tibble(Variable = vars, Description = desc) %>%
  kable()
```
...
sem_plot4 <- cowplot::plot_grid(sem_plot4, ncol = 1)
```
